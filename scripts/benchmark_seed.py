...
def _run_ortools(nodes, k, depot, dist_map, node_map,
    ...
    # Fallback 1: unseeded solve (handles bad/None seed or failed seeded solve).
    if not assign:
        assign = routing.SolveWithParameters(sp)

    # Fallback 2: relax the hard-k constraint and try again with more time.
...
def generate_pdf_report(results: List[dict], instance_name: str, k: int,
    ...
    try:
        ...
        sorted_res = sorted(results, key=lambda r: r.get("gap", float("inf")))
        valid = [r for r in sorted_res if r["valid"]]
        best_wf    = min(r.get("weighted_fairness", float("inf")) for r in valid) if valid else None
        worst_wf   = max(r.get("weighted_fairness", 0)            for r in valid) if valid else None
        ...
        with PdfPages(pdf_path) as pdf:
            ...
            if best_wf is not None:
                best_wf_algo = next((r["algo"] for r in valid
                                     if abs(r.get("weighted_fairness", 0) - best_wf) < 1e-6), "")
                ax.text(0.5, 0.10,
                        f"Best Weighted Fairness: {best_wf:.2f}  ({best_wf_algo})",
            ...
            ncols = 8
            # W_Fairness is col index 4 — highlighted with a gold header
            WF_COL = 4
            col_labels = ["Algorithm", "k", "Total Dist", "Std Dev",
                          "W_Fairness ★", "Time(s)", "Valid", "WF Gap%"]
            rows = []
            for r in sorted_res:
                rows.append([
                    r["algo"],
            ...
            nrows_t = len(rows)
            fig_h = max(6, 1.0 + nrows_t * 0.32)
            fig2, ax2 = plt.subplots(figsize=(14, fig_h))
            ax2.set_axis_off()
            ax2.set_title(
                f"{instance_name} — Algorithm Results  (sorted by Weighted Fairness gap)",
            ...
            tbl.scale(1.0, 1.6)
            # colour header — gold on W_Fairness column, dark blue on the rest
            ...
            # colour data rows by weighted fairness
            for row_i, r in enumerate(sorted_res):
                wf = r.get("weighted_fairness", float("inf"))
                if not r["valid"]:
                    fc = GREY_INV
                elif best_wf is not None and abs(wf - best_wf) < 1e-6:
                    fc = GREEN_BEST
                elif worst_wf is not None and abs(wf - worst_wf) < 1e-6:
                    fc = RED_WORST
                else:
                    fc = "#ffffff"
                for j in range(ncols):
                    tbl[row_i + 1, j].set_facecolor(fc)
                # always tint the WF cell itself
                if r["valid"] and best_wf is not None:
                    tbl[row_i + 1, WF_COL].set_facecolor(
                        "#c6efce" if abs(wf - best_wf) < 1e-6 else
                        "#fff9c4" if wf < best_wf * 1.05 else "#ffffff"
                    )
            fig2.tight_layout()
            pdf.savefig(fig2)
            plt.close(fig2)

            # ── PAGE 3: Weighted Fairness bar chart (primary) + Distance (secondary) ──
            fig_h3 = max(6, len(sorted_res) * 0.38 + 1.5)
            fig3, (ax3a, ax3b) = plt.subplots(1, 2, figsize=(18, fig_h3))

            def _hbar(ax, vals, colors, best_val, xlabel, title):
                bars = ax.barh(range(len(sorted_res)), vals, color=colors,
                               edgecolor="white", linewidth=0.5)
                ax.set_yticks(range(len(sorted_res)))
                ax.set_yticklabels([r["algo"] for r in sorted_res], fontsize=7)
                ax.invert_yaxis()
                if best_val is not None:
                    ax.axvline(best_val, color=GREEN_BAR, linestyle="--",
                               linewidth=1.5, label=f"Best: {best_val:.1f}")
                ax.set_xlabel(xlabel, fontsize=10)
                ax.set_title(title, fontsize=11, fontweight="bold")
                ax.legend(fontsize=8)
                max_v = max(vals) if vals else 1
                for bar, val in zip(bars, vals):
                    ax.text(val + max_v * 0.005, bar.get_y() + bar.get_height() / 2,
                            f"{val:.1f}", va="center", fontsize=6)
                ax.grid(axis="x", alpha=0.3)

            wf_vals = [r.get("weighted_fairness", 0) for r in sorted_res]
            wf_colors = [
                "#a9a9a9" if not r["valid"] else
                GREEN_BAR if (best_wf is not None and abs(r.get("weighted_fairness", 0) - best_wf) < 1e-6) else
                BLUE_BAR
                for r in sorted_res
            ]
            _hbar(ax3a, wf_vals, wf_colors, best_wf,
                  "Weighted Fairness  ★  (lower = better)",
                  f"{instance_name} — Weighted Fairness")

            dist_vals = [r["total"] for r in sorted_res]
            dist_colors = [
                "#a9a9a9" if not r["valid"] else
                "#70ad47" if (best_total is not None and abs(r["total"] - best_total) < 1e-6) else
                "#9dc3e6"
                for r in sorted_res
            ]
            _hbar(ax3b, dist_vals, dist_colors, best_total,
                  "Total Distance",
                  f"{instance_name} — Total Distance")

            fig3.suptitle(f"{instance_name} — Performance Overview",
            ...
            # winners panel — weighted fairness is the primary metric
            ax5b.set_axis_off()
            if valid:
                ...
                winners = [
                    ("★  Best Weighted Fairness  ★", best_wf_r["algo"],
                     f"WF = {best_wf_r.get('weighted_fairness', 0):.2f}", "#ffd966"),
...
def run_one_instance(name: str, k: int, save_outputs: bool = True,
    ...
    for aname, fn in active_algorithms:
        ...
        if sol is None:
            print(f"{aname:<42}  SKIPPED (no solution found — OR-Tools time limit or infeasible)")
            continue
    ...
    if valid:
        best_wf = min(r["weighted_fairness"] for r in valid)
        for r in results:
            wf = r["weighted_fairness"]
            r["gap"] = 100 * (wf - best_wf) / best_wf if best_wf > 0 else 0.0

    results.sort(key=lambda r: r.get("gap", float("inf")))

    for r in results:
        marker = " ***" if r.get("gap", float("inf")) < 1e-6 and r["valid"] else ""
        print(f"{r['algo']:<42}{r['k_used']:>4}{r['total']:>15.2f}"
              f"{r['std']:>12.2f}{r.get('weighted_fairness', 0):>10.2f}"
              f"{r['time']:>10.4f}  "
              f"{'OK ' if r['valid'] else 'BAD'}  {r.get('gap', 0):>6.2f}{marker}")
...
def print_winners(all_results: List[dict]):
    ...
    print("\n" + "="*90)
    print("WINNERS BY INSTANCE  (primary: Weighted Fairness  |  secondary: Distance, Std Dev)")
    print("="*90)
    for inst, rs in by_inst.items():
        ...
        if not valid:
            ...
            continue
        best_wf   = min(valid, key=lambda r: r.get("weighted_fairness", float("inf")))
        best_dist = min(valid, key=lambda r: r["total"])
        best_std  = min(valid, key=lambda r: r["std"])
        print(f"  {inst}")
        print(f"    ★ Best W_Fairness : {best_wf['algo']:<40}  WF={best_wf.get('weighted_fairness',0):.1f}")
        print(f"      Best Distance   : {best_dist['algo']:<40}  d={best_dist['total']:.1f}")
        print(f"      Best Std Dev    : {best_std['algo']:<40}  σ={best_std['std']:.1f}")
    print("="*90)
